...
void hashtable_dispose(hashtable **htptr, void (*free_key)(void *),
                       void (*free_val)(void *)) {
  if (*htptr == nullptr)
    return;
  for (size_t k = 0; k < (*htptr)->nslots; ++k) {
    ...
    while (p != nullptr) {
      cell *t = p;
      if (free_key)
        free_key((void *)p->keyref);
      if (free_val)
        free_val((void *)p->valref);
      p = p->next;
      free(t);
    }
  }
...
void hashtable_get_stats(hashtable *ht, struct hashtable_stats *htsptr) {
  size_t m = ht->nslots;
  size_t g = 0;
  double s = 0.0;
  for (size_t k = 0; k < m; ++k) {
...
#define P_TITLE(textstream, name) fprintf(textstream, "--- Info: %s\n", name)
